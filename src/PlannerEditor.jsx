...
export default function PlannerEditor({
  ...
  const saveEvent = () => run(async () => {
    ...
    await onSaveEvent({ ...eventDraft, clubId: eventDraft.clubId === 'standalone' ? null : Number(eventDraft.clubId) }, eventEditId)
    resetEvent()
  })
  ...
  return <div className="page-view manage-page">
    ...
    {tab === 'events' && <section className="editor-layout"><form className="editor-form" onSubmit={(event) => { event.preventDefault(); saveEvent() }}><div className="form-heading"><div><p className="eyebrow">{eventEditId ? 'Update a one-off item' : 'Add a one-off item'}</p><h2>{eventEditId ? 'Edit event' : 'Event details'}</h2></div></div><label>Event name<input value={eventDraft.name} onChange={(event) => setEventDraft({ ...eventDraft, name: event.target.value })} placeholder="e.g. Hackathon" /></label><div className="form-row date-row"><label>Start date<input type="date" value={eventDraft.date} onChange={(event) => setEventDraft({ ...eventDraft, date: event.target.value })} /></label><label>End date <span>optional</span><input type="date" value={eventDraft.endDate} onChange={(event) => setEventDraft({ ...eventDraft, endDate: event.target.value })} /></label></div><div className="form-row"><label>Start time<input type="time" value={eventDraft.startTime} onChange={(event) => setEventDraft({ ...eventDraft, startTime: event.target.value })} /></label><label>End time<input type="time" value={eventDraft.endTime} onChange={(event) => setEventDraft({ ...eventDraft, endTime: event.target.value })} /></label></div><label>Linked club<select value={eventDraft.clubId} onChange={(event) => setEventDraft({ ...eventDraft, clubId: event.target.value })}><option value="standalone">Standalone event</option>{clubs.map((club) => <option value={club.id} key={club.id}>{club.name}</option>)}</select></label><div className="form-actions"><button className="form-submit" type="submit" disabled={busy}>{busy ? 'Saving…' : eventEditId ? 'Save event' : 'Add event'}</button>{eventEditId && <button className="cancel-button" type="button" onClick={resetEvent}>Cancel</button>}</div></form><div className="editor-list"><div className="editor-list-heading"><div><p className="eyebrow">Dates to remember</p><h2>{events.length} events</h2></div></div>{events.length ? events.map((item) => { const club = clubs.find((entry) => entry.id === item.clubId); return <article className="editable-item detailed-item" key={item.id}><span className="item-symbol orange">E</span><div><h3>{item.name}</h3><p>{item.endDate ? item.date + ' → ' + item.endDate : item.date} · {item.startTime}–{item.endTime}</p><small>{club ? 'Linked to ' + club.name : 'Standalone event'}</small></div><div className="item-actions"><button type="button" className="edit-button" onClick={() => { setEventEditId(item.id); setEventDraft({ ...item, clubId: item.clubId == null ? 'standalone' : String(item.clubId) }); setError('') }}>Edit</button><button type="button" className="delete-button" disabled={busy} onClick={() => run(() => onDeleteEvent(item.id))}>Delete</button></div></article> }) : <p className="empty-state">No events yet. Add a date worth protecting.</p>}</div></section>}
